...
class_calcseeds::class_calcseeds(mxArray *plhs[ ],const mxArray *prhs[ ]) {
	...
	para_list = mxGetPr(prhs[10]);
	get_integer_scalar(cur_time, prhs[11]);
    //cout<<cur_time<<endl;
...
OUT class_calcseeds::calcpoint(std::vector<double> &paramvector,double &diffnorm,int &num_iterations,const int &x,const int &y,const int &num_thread) {
    ...
    OUT outstate_initialguess;
	if (cur_time == 0) {
		outstate_initialguess = initialguess(defvector_init, num_thread);
	}else{
        outstate_initialguess = SUCCESS;
        for (int ind = 0; ind < 6; ind++) {
			defvector_init[ind] = para_list[ind + 2];
		}
    }
