...
% dialogue and return.
    ...
    function constructor()                           
        ...
        for i = 1:length(current)
            im_list{i} = current(i).name;
        end
        
        %im_list = current(:).name;
        set(handles_gui.image_list,'string',im_list);
        % Set data
        ...
        set(handles_gui.figure,'WindowButtonUpFcn',ncorr_util_wrapcallbacktrycatch(@(hObject,eventdata)callback_clickplot(hObject,eventdata,false),handles_gui.figure));
        % Set Visible
    ...
    function callback_export_data(hObject,eventdata)
        [filename,pathname] = uiputfile({'*.txt'},'Save Image');
        if (~isequal(filename,0) && ~isequal(pathname,0))
                overwrite = true;
                if (exist(fullfile(pathname,filename),'file'))
                    contbutton = questdlg('File already exists. Do you want to overwrite?','Continue Operation','Yes','No','No');
    ...
    function callback_detailplot(hObject,eventdata,detail) %#ok<INUSD>
        % Get data
    ...
    function callback_listbox_change(hOject,eventdata) %#ok<INUSD>
        num_cur = getappdata(handles_gui.figure,'num_cur');  
        friends = getappdata(handles_gui.figure,'friends'); 
        ...
        A= get(handles_gui.image_list,'value');
        setappdata(handles_gui.figure,'num_cur',A-1);
        update_axes('set',handles_gui);     
        update_sidemenu(handles_gui);    
    end
    ...
    function update_axes(action,handles_gui_sub)
        ...
        %before here lore has not been defined        
        if (val_popupmenu == 1)
            lore = 'lagrangian';
            img_bg = reference;
        else
    ...
    % This function forms the axes seen at the top left of the plot  
        % Get data draw top left tuxing,zheng feijing
        num_cur = getappdata(handles_gui_sub.figure,'num_cur');       
        val_popupmenu = getappdata(handles_gui_sub.figure,'val_popupmenu');
    ...
    function callback_clickplot(hObject,eventdata,detail) %#ok<INUSD>
    % This function tells the axes what to do if the mouse cursor hovers
    % over the plot axes
        % Get data
        num_cur = getappdata(handles_gui.figure,'num_cur');
        ...
        pos_mouse_axes = round(get(handles_gui.axes_formatplot,'CurrentPoint')); 
        if (detail)
            plot(handles_gui.plot_for_x,handles_gui.plot_for_y);
            legend(strcat('Time -  ',text_display));
            xlabel('Time');
            ylabel(strcat(text_display,units));             
            return
        end
        if (pos_mouse_axes(1) >= 1 && pos_mouse_axes(3) >= 1 && pos_mouse_axes(1) <= size(roi_data.mask,2) && pos_mouse_axes(3) <= size(roi_data.mask,1))
            if (roi_data.mask(pos_mouse_axes(3),pos_mouse_axes(1)))
                %set(text_info,'String',{['x-pos: ' num2str(pos_mouse_axes(1))],['y-pos: ' num2str(pos_mouse_axes(3))],[text_display num2str(plot_data(pos_mouse_axes(3),pos_mouse_axes(1))) 'change ' units]}, ...
                %    'Position',[pos_mouse_axes(1)+30 pos_mouse_axes(3)+30 2],'HorizontalAlignment','left','VerticalAlignment','top');
                x = 1:num_cur;
                y = x;
                for iter =1:num_cur
                    plot_data = get_dataplot(type_plot,lore,iter);
                    y(iter) = plot_data(pos_mouse_axes(3),pos_mouse_axes(1));
                end
                plot( handles_gui.axes_clickplot,x,y);
                legend(handles_gui.axes_clickplot,strcat('Time -  ',text_display));
                xlabel(handles_gui.axes_clickplot,'Time');
                ylabel(handles_gui.axes_clickplot,strcat(text_display,units));
                handles_gui.plot_for_x = x;
                handles_gui.plot_for_y = y;
                set(handles_gui.button_save_img,'Enable','on');
                set(handles_gui.button_export_data,'Enable','on');
            else
                % Clear text and set cursor back to arrow if zoom and pan
                % aren't enabled
                set(text_info,'String','');
    ...
    function handles_gui = init_gui()        
        handles_gui.figure = figure( ...
            'Tag', 'figure', ...
            'Units', 'characters', ...
            'Position', ncorr_util_figpos(pos_parent,[44.5 265]), ...
            'Name', 'Data Plot', ...
            'MenuBar', 'none', ...
            'NumberTitle', 'off', ...
        ...
        handles_gui.group_menu = uipanel( ...
            ...
            'Tag', 'group_menu', ...
            'Units', 'characters', ...
            'Position', [2 23.4 5.0 2.6], ...
            'Title', 'Local Plot Options', ...
            'Interruptible','off',...
            'Visible','off');

        handles_gui.plot_info = uipanel( ...
            'Parent', handles_gui.figure, ...
            'Tag', 'plot_info', ...
            'Units', 'characters', ...
            'Position', [150.0 0.8 109 43.2], ...
            'Title', 'plot', ...
            'Interruptible','off');
        ...
        handles_gui.group_zoompan = uibuttongroup( ...
            ...
            'Tag', 'group_zoompan', ...
            'Units', 'characters', ...
            'Position', [2 27 35.0 4.4], ...
            'Title', 'Zoom/Pan', ...
            'Interruptible','off');
        
        
        %handles_gui.image_list = ui
        % Axes
        ...
        handles_gui.axes_clickplot = axes( ...
            'Parent', handles_gui.plot_info, ...
            'Tag', 'axes_clickplot', ...
            'Units', 'characters', ...
            'Position', [12.4 19.0 86.2 20], ...
            'Interruptible','off');
        ...
        %list box
        handles_gui.image_list = uicontrol( ...
            'Parent', handles_gui.figure, ...
            'Tag', 'image_listbox', ...
            'Style', 'listbox', ...
            'Units', 'characters', ...
            'Position', [2 0.8 35 22], ...
            'BackgroundColor', [1 1 1], ...
            'String', {}, ...
            'Callback', ncorr_util_wrapcallbacktrycatch(@callback_listbox_change,handles_gui.figure), ...
            'Interruptible','off');
        ...
        % Now get info text which is below the plot
        handles_infotext = set_infotext(handles_gui.group_formataxes,handles_gui.plot_info,'');
        fields_infotext = fieldnames(handles_infotext);
        for i = 0:length(fields_infotext)-1
        ...
        handles_gui.button_save_img = uicontrol( ...
            'Parent', handles_gui.group_formataxes, ...
            'Tag', 'save_img', ...
            'Style', 'pushbutton', ...
            'Units', 'characters', ...
            'Position', [65 1.0 16 1.8], ...
            'String', 'export image', ...
            'Callback', ncorr_util_wrapcallbacktrycatch(@(hObject,eventdata)callback_topmenu_save(hObject,eventdata,false),handles_gui.figure), ...
            'Interruptible','off');
        ...
        handles_gui.button_save_img = uicontrol( ...
            ...
            'Style', 'pushbutton', ...
            'Units', 'characters', ...
            'Position', [65 1.0 15 1.8], ...
            'String', 'detailed plot', ...
            'Callback', ncorr_util_wrapcallbacktrycatch(@(hObject,eventdata)callback_clickplot(hObject,eventdata,true),handles_gui.figure), ...
            'Interruptible','off',...
            'Enable', 'off');
        
        handles_gui.button_export_data = uicontrol( ...
            'Parent', handles_gui.plot_info, ...
            'Tag', 'detailed plot', ...
            'Style', 'pushbutton', ...
            'Units', 'characters', ...
            'Position', [83 1.0 15 1.8], ...
            'String', 'export data', ...
            'Callback', ncorr_util_wrapcallbacktrycatch(@(hObject,eventdata)callback_export_data(hObject,eventdata),handles_gui.figure), ...
            'Interruptible','off',...
            'Enable', 'off');
    end
end
...
function handles_infotext = set_infotext(parent,parent2,bgcolor)
